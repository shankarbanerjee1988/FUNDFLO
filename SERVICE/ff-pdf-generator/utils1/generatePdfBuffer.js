// Modified generatePdfBuffer.js without garbage collection calls (continued)
...
// Enhanced footer template with optional custom text
const enhancedFooterTemplate = `
...
// Update the default options in generatePdfBuffer.js
const defaultOptions = {
  format: 'A4',
  landscape: false,
  printBackground: true,
  displayHeaderFooter: true,
  headerTemplate: enhancedHeaderTemplate,
  footerTemplate: enhancedFooterTemplate,
  margin: { top: "100px", right: "20px", bottom: "80px", left: "20px" },
  preferCSSPageSize: false,
  timeout: 30000 // 30 second timeout for PDF generation
};

// Function to wait briefly for async operations to complete
async function waitForOperations(ms = 100) {
...
module.exports = async function generatePdfBuffer(html, options = {}) {
  ...
  // Define the defaultStyles first
  const defaultStyles = `
  <style>
      body { 
...
try {
  ...
  // Wait briefly before generating the PDF
  await waitForOperations(100);
